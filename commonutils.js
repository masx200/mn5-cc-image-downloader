...
async function downloadallpagesfromdocument(document) {
    ...
    await Promise.all(selectpagehtmlurls(document).map(async url => {
        const dom = await resolvedocumentfromurl(url);
        return await downloadonepageallimages(dom);
    }));
    console.log('all album\xA0images\xA0download\xA0done ' + document.documentURI);
}
...
function selectimagesfromdocument(document) {
    const fileurls = Array.from(new Set(Array.from(document.querySelectorAll('img')).map(e => {
        return new URL(e.getAttribute('src'), document.documentURI).href;
    }).filter(a => !!a).filter(a => a.startsWith('http'))));
    return fileurls;
}
async function downloadonepageallimages(document) {
    ...
    console.log('one page\xA0images\xA0download\xA0done ' + document.documentURI);
}
function selectpagehtmlurls(document) {
    return Array.from(new Set(Array.from(document.querySelectorAll('.page > a')).map(e => {
